...
// Parámetros para el cambio de carril de los obstáculos
const OBSTACLE_LANE_CHANGE_COOLDOWN_MIN = 0.8;  // segundos
const OBSTACLE_LANE_CHANGE_COOLDOWN_MAX = 2.2;
const OBSTACLE_LANE_CHANGE_PROB = 0.45;          // probabilidad por tick cuando el cooldown llega a 0
const OBSTACLE_LANE_LERP_SPEED = 4.5;            // velocidad de interpolación (más alto = más rápido)
  ...
  useEffect(() => {
    ...
    const step = (nowMs: number) => {
      ...
      if (spawnTimerMsRef.current >= SPAWN_EVERY_MS) {
        ...
        // 🔥 NUEVO: inicializar los atributos para cambio de carril
        obstaclesRef.current.push({
          id,
          laneX,
          targetLaneX: laneX,
          changeCooldown: OBSTACLE_LANE_CHANGE_COOLDOWN_MIN +
            Math.random() * (OBSTACLE_LANE_CHANGE_COOLDOWN_MAX - OBSTACLE_LANE_CHANGE_COOLDOWN_MIN),
          size,
          z: OBSTACLE_SPAWN_Z,
          shape,
        });
      ...
      // Spawn de estrellas (sin cambios)
      ...
      // Spawn de pirámides (sin cambios)
      ...
      // Actualización de proyectiles y destrucción de obstáculos
      ...
      // 🔥 ACTUALIZACIÓN DE OBSTÁCULOS CON MOVIMIENTO LATERAL
      const newObstacles: Obstacle[] = [];
      for (const o of obstaclesRef.current) {
        // 1. Avanzar en Z
        const newZ = o.z + speed * dt;

        // 2. Gestionar cooldown de cambio de carril
        let newChangeCooldown = o.changeCooldown - dt;
        let newTargetLaneX = o.targetLaneX;

        if (newChangeCooldown <= 0) {
          // Probabilidad de cambiar de carril
          if (Math.random() < OBSTACLE_LANE_CHANGE_PROB) {
            // Escoger un carril diferente al actual
            const currentIdx = LANES.indexOf(o.targetLaneX);
            let newIdx = currentIdx;
            while (newIdx === currentIdx) {
              newIdx = Math.floor(Math.random() * LANES.length);
            }
            newTargetLaneX = LANES[newIdx];
          }
          // Reiniciar cooldown (aunque no haya cambiado, se reinicia para evitar cambios constantes)
          newChangeCooldown = OBSTACLE_LANE_CHANGE_COOLDOWN_MIN +
            Math.random() * (OBSTACLE_LANE_CHANGE_COOLDOWN_MAX - OBSTACLE_LANE_CHANGE_COOLDOWN_MIN);
        }

        // 3. Interpolar posición X hacia el target
        let newLaneX = o.laneX;
        if (Math.abs(newTargetLaneX - o.laneX) > 0.001) {
          newLaneX += (newTargetLaneX - o.laneX) * Math.min(1, OBSTACLE_LANE_LERP_SPEED * dt);
        } else {
          newLaneX = newTargetLaneX;
        }

        // 4. Actualizar el elemento visual si existe
        const el = obstacleElsRef.current.get(o.id) as AFrameEntity | undefined;
        if (el?.object3D) {
          el.object3D.position.z = newZ;
          el.object3D.position.x = newLaneX;
        }

        // 5. Colisión con el jugador
        if (newZ <= OBSTACLE_DESPAWN_Z) {
          const half = o.size / 2;
          const dz = Math.abs(newZ - PLAYER_Z);
          const dx = Math.abs(newLaneX - playerX);
          ...
          if (colliding && !hitOccurred && hitCooldownRef.current <= 0) {
            ...
            newObstacles.push({
              ...o,
              laneX: newLaneX,
              targetLaneX: newTargetLaneX,
              changeCooldown: newChangeCooldown,
              z: newZ,
            });
          }
        } else {
          // Si el obstáculo todavía no ha pasado al jugador, se mantiene
          newObstacles.push({
            ...o,
            laneX: newLaneX,
            targetLaneX: newTargetLaneX,
      ...
      // Actualización de estrellas (sin cambios)
      const newStars: Star[] = [];
      ...
      // Actualización de pirámides (sin cambios)
      ...
      // Forzar actualizaciones de versión para renderizado
